...
const Payable = ({ id, invoiceType }) => {
    ...
    <div className="custom-dropdown-content">
      <CRow>
        <div class="logo-dropdown">
          <select id="dropdown" class="headertext-dropdown">
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (
          responseInvoices.status === 200 &&
          Array.isArray(responseInvoices.data.data)
        ) {
  ...
  const toggleCardVisibility = async (id) => {
    ...
    try {
      const responseTransaction = await axios.get(
        `http://localhost:8080/api/v1/invoice/invoice-detail/${id}`
      );
  ...
  const handleSendMessage = () => {
    if (newMessage.trim()) {
      setMessages([
        ...messages,
        { id: Date.now(), text: newMessage, type: "sent", sender: "You" },
      ]);
      setNewMessage("");
    }
  };
        ...
        <div>
          <CTooltip content="Details">
            <CButton
              onClick={(e) => {
                e.preventDefault();
                toggleCardVisibility(row.InvoiceID);
              }}
              style={{
                background: "none",
                border: "none",
    ...
    <div className="Payable">
      <CCard className="card card-payable">
        <div className="sum-card">
          <div className="invoice-text">
            <CCardBody className="text-invoicepay">
              Scheduled Payments
            </CCardBody>
          </div>
      ...
      `}</style>
        ...
        >
          ...
          <Tab eventKey="Awaiting Approval" title="Awaiting Approval"></Tab>
          <Tab eventKey="Awaiting Payment" title="Awaiting Payment"></Tab>
        </Tabs>
      </div>
            ...
            <div>
                ...
                <>
                  <div>
                    ...
                    <p className="sydtime-text">
                      {transactionDetail.Date} (SYD/MEL Time)
                    </p>
                    <p className="sydtime-text">
              ...
              <div>
                ...
                <p class="dumpy-text">
                  ...
                  Duis congue velit elit, at accumsan nisi malesuada fermentum.{" "}
                </p>
                <div class="border-bottom"></div>
                <div class="buton-edit"></div>
                {!showDropdown ? (
                  ""
                ) : (
                  <div>
                    ...
                    <CTable className="rotate-table">
                      <CTableHead>
                        {historydata.map((item, index) => (
                          <CTableRow key={index}>
                            <CTableHeaderCell className="querytable-text">
                              {item.EditBy}
                            </CTableHeaderCell>
                            <CTableHeaderCell className="valuetable-text">
                              {item.date}
                            </CTableHeaderCell>
                          </CTableRow>
                        ))}
