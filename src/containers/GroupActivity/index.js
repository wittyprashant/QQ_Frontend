...
class GroupActivity extends Component {
    ...
    columnGroup: [
      ...
      },
      {
        ...
        cell: (row) => {
          return (
            <div>
              <CTooltip content="Activity Edit">
                <button
                  type="button"
                  onClick={(e) => {
                    this.openLink(row.taskId);
                  }}
                  className="btn btn-info btn-sm  ms-2 mt-2"
                >
                  <CIcon size={"sm"} icon={cilPen} />
                </button>
              </CTooltip>
              <CTooltip content="Activity Delete">
                <button
                  type="button"
                  onClick={(e) => {
                    this.deleteGroupActivity(row.taskId);
                  }}
  ...
  openLink(taskId) {
    ...
    axios
      .get(
        "SuperAdmin/checktaskdayslimit?groupId=" +
      ...
      .then((response) => {
        if (response.data.status) {
          ...
          if (taskId) {
            Link =
              "/group_activity_management/edit/" +
              window.location.pathname.split("/").pop() +
              "/" +
  ...
  deleteGroupActivity(id) {
    swal({
      ...
      if (willDelete) {
        ...
        axios
          .delete("SuperAdmin/deletegroupactivitytask?Id=" + id, headers)
          .then((response) => {
            if (response.data.status) {
              const token = this.props.token;
              const param = {
...
const mapDispatchToProp = (dispatch) => {
  return {
    onGroupActivity: (param, token) =>
      dispatch(actions.groupActivity(param, token)),
    onGroupActivityDelete: (id, token) =>
      dispatch(actions.groupActivityDelete(id, token)),
  };
};
