...
function withRouter(Component) {
  return (props) => {
    ...
    const match = useMatch(
      "/group_activity_management/edit/:categoryName/:groupId/:maxRows/:taskId"
    );
    return (
      <Component
        {...props}
...
class AddEdit extends Component {
  ...
  componentDidMount() {
    ...
    if (taskId) {
      ...
      axios
        .get("SuperAdmin/getgroupactivitytaskbyid?Id=" + taskId, headers)
        .then((response) => {
      ...
      <CCard>
        <CCardHeader className="d-flex" component="h5">
          ...
          <div className="col-sm">
            Group Task {this.state.taskId ? "Edit" : "Add"}
          </div>
        </CCardHeader>
        <CCardBody>
          <CForm
...
const mapDispatchToProp = (dispatch) => {
  return {
    onAddEditForm: (params, token, link) =>
      dispatch(actions.groupActivityAddEdit(params, token, link)),
    onGroupActivitygetById: (Id, token) =>
      dispatch(actions.groupActivitygetById(Id, token)),
  };
};
