  ...
  state = {
    ...
    columnReplay: [
      ...
      },
      {
        ...
        width: "200px",
        cell: (row) => {
          return <span>{Moment(row.createOn).format("DD-MM-YYYY")}</span>;
        },
      },
      {
        name: "Actions",
        ...
        cell: (row) => {
          return (
            <div>
              <CTooltip content="Edit Replays">
                <NavLink
                  to={`/replays/edit/${row.faqId}`}
                  className="btn btn btn-sm btn-info mt-2 me-1"
                >
                  <CIcon size={"sm"} icon={cilPen} />
                </NavLink>
              </CTooltip>
              <CTooltip content="Delete Replays">
                <button
                  type="button"
                  onClick={(e) => {
                    this.deleteReplay(row.id);
                  }}
