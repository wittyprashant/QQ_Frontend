  ...
  state = {
    ...
    columnDigital: [
      ...
      },
      {
        ...
        width: "150px",
        cell: (row) => {
          return <span>{Moment(row.createdOn).format("DD-MM-YYYY")}</span>;
        },
      },
      {
        name: "Actions",
        ...
        cell: (row) => {
          return (
            <div>
              <CTooltip content="Edit Digital Library">
                <NavLink
                  to={`/digital_library/edit/${row.digitalLibraryId}`}
                  onClick={(e) => {
                    this.GetbyFundingforms(row.fundingId);
                  }}
              ...
              <CTooltip content="Delete Digital Library">
                <button
                  type="button"
                  onClick={(e) => {
                    this.deleteDigitalLibrary(row.digitalLibraryId);
                  }}
      ...
      <CCard>
        <CCardHeader className="d-flex" component="h5">
          ...
          <div className="col-sm text-end">
            <CTooltip content="Add Digital Library">
              <NavLink
                to={"/digital_library/add/"}
                type="button"
...
const mapDispatchToProp = (dispatch) => {
  return {
    onDigitalLibraryList: (param) =>
      dispatch(actions.digitalLibraryList(param)),
    onDigitalLibraryFilterList: (text) =>
      dispatch(actions.digitalLibraryListFilter(text)),
    onDigitalLibraryDelete: (id, token) =>
      dispatch(actions.digitalLibraryDelete(id, token)),
  };
};
