...
export const UserRequestList = (param, token) => {
  return (dispatch) => {
    ...
    axios
      .get("SuperAdmin/getuserrolereqlist")
      .then((response) => {
...
export const UserRequestUpdate = (reqID, statusId, memberId, roleId) => {
  return (dispatch) => {
    ...
    axios
      .get("SuperAdmin/acceptrejectreq?reqId=" + reqID + "&status=" + statusId)
      .then((response) => {
        if (statusId === 1) {
          axios
            .get(
              "SuperAdmin/changeuserroleid?memberId=" +
                memberId +
                "&roleId=" +
                roleId
            )
...
export const UserRequestDetail = (memberId) => {
  return (dispatch) => {
    axios
      .get("SuperAdmin/getmemberqueansbyid?memberId=" + memberId)
      .then((response) => {
        const data = response.data.result;
