...
const roleDetailFail = (state, action) => {
  return updateObject(state, {
    roleDetailError: action.error,
    roleDetailDatail: intialState.defaultRolePermisionData,
    roleDetailLoading: false,
  });
};
...
const reducer = (state = intialState, action) => {
  switch (action.type) {
    case actionTypes.ROLE_ADD_EDIT_SUCCESS:
      return addEditSuccess(state, action);
    case actionTypes.ROLE_ADD_EDIT_FAIL:
      return addEditFail(state, action);
    case actionTypes.ROLE_ADD_EDIT_START:
      return addEditStart(state, action);
    case actionTypes.ROLE_DETAIL_START:
      return roleDetailStart(state, action);
    case actionTypes.ROLE_DETAIL_FAIL:
      return roleDetailFail(state, action);
    case actionTypes.ROLE_DETAIL:
      return roleDetailSuccess(state, action);
    case actionTypes.ROLE_ADD_EDIT_SUCCESS_MESSAGE:
      return addEditSuccessMessage(state, action);
    default:
      return state;
  }
};
