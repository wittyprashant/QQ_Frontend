...
const eventDetail = (state, action) => {
    return updateObject(state, { eventDetail: action.data, eventDetailError: action.error })
}
...
const userListFail = (state, action) => {
    return updateObject(state, { userListLoading: false, userListError: action.error, data: [] })
}
...
const userListing = (state, action) => {
    return updateObject(state, {
        userListLoading: false,
        userListError: null,
        userListData: action.page === 1 ? action.data : [...state.data, ...action.data],
    })
}
...
export const eventReducer = (state = intialState, action) => {
    switch (action.type) {
        case actionTypes.EVENT_LIST_START: return eventListStart(state, action)
        case actionTypes.EVENT_LIST_FAIL: return eventListFail(state, action)
        case actionTypes.EVENT_LIST_SUCCESS: return eventListing(state, action)
        case actionTypes.EVENT_DETAIL: return eventDetail(state, action)
        case actionTypes.EVENT_ADD_EDIT_SUCCESS: return addEditSuccess(state, action)
        case actionTypes.EVENT_ADD_EDIT_FAIL: return addEditFail(state, action)
        case actionTypes.EVENT_ADD_EDIT_START: return addEditStart(state, action)
        case actionTypes.EVENT_COUNTRY_LIST: return countryList(state, action)
        case actionTypes.EVENT_STATE_LIST: return stateList(state, action)
        case actionTypes.EVENT_USER_LIST_START: return userListStart(state, action)
        case actionTypes.EVENT_USER_LIST_FAIL: return userListFail(state, action)
        case actionTypes.EVENT_USER_LIST_SUCCESS: return userListing(state, action)
        case actionTypes.EVENT_LIST_FILTER: return filterList(state, action)
