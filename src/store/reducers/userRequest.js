...
const QuetionData = {
  meetQuickQuery: "Where did you meet Quick Query?",
  entrepreneur: "Are you an entrepreneur?",
  investor: "Are you an investor?",
  cannabisIndustry:
    "What sector of the cannabis industry do you currently work in?",
  ...
  donorSponsorQuickQuery:
    "Are you interested in being a donor/ sponsor of Quick Query?",
  valueGender: "Gender",
  bipoc: "Is the company 51% BIPOC owned?",
  raceEthnicity: "Race/Ethnicity",
  iDentifyRaceEthnicity:
    "Please further identify your race/ethnicity [ie. You may have checked Asian but what origin do you associate with: Chinese, Indian, Filipino, Vietnamese, Korean, Japanese, etc]",
  ...
  enrolledOtherServicesAdministered:
    "Are you enrolled in any other services administered for cannabis support? If so, which program(s)?",
  aboutYourBrand: "Please tell us more about your brand.",
  meetNuTeam:
    "If you are interested in meeting with a Quick Query team member, please share times you have available in the upcoming weeks and an email to best contact you.",
  otherRemarks:
    "Anything additional you may need to add or share please leave here.",
  historicallyDisadvantaged:
    "My business is 51% or greater historically disadvantaged owned (minority, women or veteran) OR my business is considered a “small business.” or businesses identifying as historically disadvantaged please mark all that apply:",
  signupAsA: "Sign up as a",
  knowledgeBuddingEntrepreneurs:
    "If an expert, what area of the industry can you share knowledge about to budding entrepreneurs? ",
...
const userRequestDetail = (state, action) => {
  ...
  Object.keys(userRequestResponse).map(function (key, x) {
    if (typeof QuetionData[key] !== "undefined") {
      result.push({
        Quetion: QuetionData[key],
        Answer: userRequestResponse[key] ?? "-",
      });
    }
...
const filterList = (state, action) => {
  ...
  let filteredItems = fundData.filter((item) => {
    if (
      ...
      ((item.status_data &&
        item.status_data.toLowerCase() === status.toLowerCase()) ||
        status === "")
    ) {
      return true;
    }
...
const reducer = (state = intialState, action) => {
  switch (action.type) {
    case actionTypes.USER_REQUEST_LIST_START:
      return userRequestListStart(state, action);
    case actionTypes.USER_REQUEST_LIST_FAIL:
      return userRequestListFail(state, action);
    case actionTypes.USER_REQUEST_LIST_SUCCESS:
      return userRequestListing(state, action);
    case actionTypes.USER_REQUEST_UPDATE_START:
      return userRequestUpdateStart(state, action);
    case actionTypes.USER_REQUEST_UPDATE_FAIL:
      return userRequestUpdateFail(state, action);
    case actionTypes.USER_REQUEST_STATUS_UPDATE:
      return userRequestUpdate(state, action);
    case actionTypes.USER_REQUEST_DETAIL:
      return userRequestDetail(state, action);
    case actionTypes.USER_REQUEST_LIST_FILTER:
