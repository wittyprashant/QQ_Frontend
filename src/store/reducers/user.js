...
const filterList = (state, action) => {
  ...
  let filteredItems = fundData.filter((item) => {
    if (
      ...
      ((item.roleName && item.roleName.toLowerCase() === role.toLowerCase()) ||
        role === "")
    ) {
      return true;
    }
...
const reducer = (state = intialState, action) => {
  switch (action.type) {
    ...
    case actionTypes.USER_PLAN_LIST:
      return planList(state, action);
    case actionTypes.USER_ROLE_LIST:
      return roleList(state, action);
    case actionTypes.USER_ADD_EDIT_SUCCESS:
      return addEditSuccess(state, action);
    case actionTypes.USER_ADD_EDIT_FAIL:
      return addEditFail(state, action);
    case actionTypes.USER_ADD_EDIT_START:
      return addEditStart(state, action);
    case actionTypes.USER_LIST_FILTER:
